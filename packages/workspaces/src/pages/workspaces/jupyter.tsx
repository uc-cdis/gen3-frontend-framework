      ...
      {isRemoteTierPage && gatewayUnavailable ? (
        <div className="flex h-full min-h-[40vh] w-full items-center justify-center px-6 py-10">
          <div className="max-w-2xl rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            Remote kernel gateway is temporarily unavailable. Please retry in a
            moment or use the Jupyter Lite workspace while the backend is
            starting.
          </div>
        </div>
      ) : isDevelopment ? (
        <HostedWorkspace
          ...
          microContainerConfig={{
            identifierTag:
              process.env.NEXT_PUBLIC_MICRO_CONTAINER_TAG ||
              'micro-notebook-dev',
          }}
