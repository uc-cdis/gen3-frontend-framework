...
import { Badge, Loader, Button } from '@mantine/core';
import type { MicroContainerStatus } from '../hooks/useMicroContainer';
...
const MicroContainerPanel = ({
  ...
  if (status === 'not-running' || status === 'unknown') {
    return (
      <div className="flex flex-col items-center justify-center gap-6 py-12 px-6">
        ...
        {lastError && (
          <p role="alert" className="rounded-md border border-primary-light/70 bg-primary-max px-4 py-2 text-xs text-primary">
            {lastError}
          </p>
        )}
      </div>
  ...
  if (status === 'launching') {
    ...
    return (
      <div role="status" className="flex flex-col items-center justify-center gap-6 py-12 px-6">
        <Loader size={48} />
  ...
  if (status === 'error') {
    return (
      <div role="alert" className="flex flex-col items-center justify-center gap-4 py-12 px-6">
    ...
    return (
      ...
      <div className="flex items-center justify-between gap-3 rounded-lg border border-accent-light/60 bg-accent-max px-3 py-2">
        <span className="flex items-center gap-1.5 text-xs font-semibold text-accent-dark">
          <span className="h-1.5 w-1.5 rounded-full bg-accent-dark" aria-hidden="true" />
          Workspace running
        </span>
        <button
          type="button"
