...
import { isExpired, JWTPayloadAndUser } from "../../pages/api/auth/sessionToken";

const fetcher = (url: string) => fetch(url).then(r => r.json() as Promise<AuthTokenData>);

export const useAuthSession = (): AuthTokenData => {
  const { data: sessionToken, error } = useSWR<AuthTokenData>("/api/auth/sessionToken", fetcher);
  console.log("useAuthSession", sessionToken, typeof window === "undefined" ? "server" : "client");
  return error || sessionToken === undefined ? { status: "error" } : sessionToken;
}
