...
const toGlyph = (category?: string) => {
  ...
  const markerByCategory: Record<string, React.ReactNode> = {
    administrative: (
      <IconBuildingBank stroke={2.4} className={markerClassName} />
    ),
    analysis: <IconChartBar stroke={2.4} className={markerClassName} />,
    clinical: <IconPlus stroke={2.8} className={markerClassName} />,
    'experimental-methods': (
      <IconBinaryTree2 stroke={2.4} className={markerClassName} />
    ),
    'data-file': (
      <IconFileDescription stroke={2.4} className={markerClassName} />
    ),
    biospecimen: <IconTestPipe2 stroke={2.4} className={markerClassName} />,
    'medical-history': (
      <IconReportMedical stroke={2.4} className={markerClassName} />
    ),
    default: <IconDna2 stroke={2.4} className={markerClassName} />,
  };

  return (
    markerByCategory[normalizeCategory(category)] ?? markerByCategory.default
  );
};
  ...
  const rows = Object.entries(properties).map(([property, definition]) => {
    const description = String(
      definition.description ??
        (definition.term as { description?: string } | undefined)
          ?.description ??
        'No Description',
    );
    ...
    return [
      property,
      types,
      required.includes(property) ? 'Required' : 'No',
      description,
    ]
...
const GraphView = ({
  ...
  const selectedStructure = useMemo(() => {
    ...
    const levelById = graph.rows.reduce<Record<string, number>>(
      (acc, row, rowIndex) => {
        row.forEach((node) => {
    ...
    const pickPrimaryParent = (nodeId: string) => {
      ...
      const upstreamParents = parents.filter((parent) => {
        const parentLevel =
          levelById[parent.parentId] ?? Number.MAX_SAFE_INTEGER;
    ...
    const nodes = Array.from(upstreamIds)
      ...
      .sort((left, right) => {
        const levelDiff =
          (levelById[left.id] ?? 0) - (levelById[right.id] ?? 0);
  ...
  useEffect(() => {
    ...
    const updateConnectors = () => {
      ...
      const nextConnectors = graph.edges
        .map((edge) => {
          ...
          const startX =
            (sourceRect.left - layoutRect.left + sourceRect.width / 2) /
            contentScale;
          const startY = (sourceRect.bottom - layoutRect.top) / contentScale;
          const endX =
            (targetRect.left - layoutRect.left + targetRect.width / 2) /
            contentScale;
    ...
    scrollElement?.addEventListener('scroll', updateConnectors, {
      passive: true,
    });
  ...
  useEffect(() => {
    ...
    const updatePopupPosition = () => {
      ...
      const left =
        (cardRect.left - layoutRect.left + cardRect.width / 2) / contentScale;
      ...
      setPopupPosition({
        left: clamp(
          left - 170,
          16 / contentScale,
          Math.max(maxLeft, 16 / contentScale),
        ),
        top: clamp(
          top,
          24 / contentScale,
          (viewportRect.height - 240) / contentScale,
        ),
      });
    };
    ...
    <div className="relative h-full overflow-hidden bg-[#f7f8fb]">
      ...
      <div className="absolute left-6 top-6 z-30 flex flex-col gap-3">
        <ActionIcon
          ...
          size={46}
          onClick={() =>
            setZoomLevel((value) => clamp(value + ZOOM_STEP, 0.6, 3))
          }
        >
          <IconPlus size={28} />
        </ActionIcon>
                    ...
                    >
                      {row.map((node) => {
                        const isSelected =
                          ...
                          node.id === selectedNodeFromTable;
                        const isHighlighted = selectedStructureNodeIds.has(
                          node.id,
                        );
              ...
              <div className="flex items-center justify-between bg-[#545454] px-6 py-4 text-white">
                <div className="flex items-center gap-3">
                  <span
                    ...
                    style={{
                      backgroundColor: activeNodeStyle.icon,
                      color: 'white',
                    }}
                  >
