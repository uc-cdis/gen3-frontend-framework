...
const ResultList = ({ matches, term }: ResultListProps) => {
    return (<>
        {matches?.length && (<div className="flex flex-col border border-solid border-black rounded-t-md border-b-0">
            ...
            {matches.map(({ node, category, property }: { node: string, category: string, property: string }) => {
                return (<div className="flex w-full p-0.5"><div className="text-xs whitespace-nowrap overflow-hidden overflow-ellipsis" >{node}{" > "}{category}{" > "}{property}</div></div>)
            })}
        </div>)}
    </>)
}
