...
const SectionCollapse = ({ text, children } : PropsWithChildren<SectionCollapseProps>) => {
  const [expanded, { toggle }] = useDisclosure(true);


  return (
    <>
      <div className={`flex items-center gap-2 w-full bg-primary-lightest p-2 ${!expanded ? 'mb-4' : ''}`} onClick={toggle} onKeyDown={createKeyboardAccessibleFunction(toggle)} role="button" tabIndex={0}>
        <div className="rounded-full w-6 h-6 bg-primary-darker flex justify-center items-center">
          {expanded ? <UpArrowIcon color="white" size={30} /> : <DownArrowIcon color="white"  size={30} />}
        </div>
        <Text>{text}</Text>
      </div>
