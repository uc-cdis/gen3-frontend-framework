...
const AnalysisCenter = ({
    analysis,
}: AnalysisCenterProps): ReactElement => {
    const [cards, setCards] = useState({ type: "" as string, access: "" as string, search: "" });

    const handleCardsFilter = (filterType: 'type' | 'access', selected: string | null) => {
    ...
    useEffect(() => { console.log('center', centerList) }, []);
    ...
    return <div className="w-full bg-gray-100">
        ...
        <div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 p-1 mx-4 overflow-y-scroll">
                {centerList.map(({ title, description, type, icon, image, loginRequired }, key) => {
                    return <div key={key} className="flex flex-col items-center">
                        <div className="rounded-sm bg-white">
                            <div className="p-0 rounded-sm">
                                <Image src={`/images/apps/${image}`}
                                    alt="todo"
                                    height={1000}
                                    width={1000}
                                />
                            ...
                            <div className="flex flex-col mt-2 ml-2">
                                ...
                                <div className="text-xs text-gray-400 h-6">{loginRequired ? 'Login Required' : ' '}</div>
                                <div className="text-sm p-2 h-fit">
                                    <TextDescription description={description} />
                                </div>
                                <div className="flex mb-4 rounded-b-md h-6">
                                    {type === 'application' ? (<div className="m-auto">
                                        <button className="bg-blue-500 mr-2 text-white p-1.5 rounded-sm text-sm font-semibold">Run App</button>
                                        <button className="bg-blue-500 ml-2 text-white p-1.5 rounded-sm text-sm font-semibold">Demo</button>
                                        </div>) : (<button className="m-auto bg-blue-500 text-white p-1.5 rounded-sm text-sm font-semibold">View Notebook</button>)}
                                </div>
                            </div>
                        </div>
                    </div>
