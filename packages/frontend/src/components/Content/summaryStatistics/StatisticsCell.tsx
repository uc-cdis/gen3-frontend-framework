...
const StatisticsCell = ({ info, index, total }: StatisticsCellProps) => {
  ...
  const fmt = info.format ?? humanFormat;

  return (
    <div className={['flex flex-col gap-1 px-4 py-5'].join(' ')}>
      <Text className="font-mono text-sm tracking-widest uppercase text-base-contrast-darker">
        {info.label}
      </Text>
      <Text className="font-header text-3xl font-light tracking-tight leading-none text-base-contrast-darker">
        {fmt(raw)}
      </Text>
      <Text className="text-md font-light text-accent">{info.unit}</Text>
    </div>
  );
};
