...
export const isRootUrl = (urlFragment: string): boolean =>
  urlFragment === '_root';

export const isProgramUrl = (urlFragment: string): boolean =>
  urlFragment !== '_root' && !urlFragment.includes('-');

export const userHasSheepdogProgramAdmin = (
  userAuthMapping: Record<string, any> = {},
): boolean =>
  userAuthMapping['/services/sheepdog/submission/program'] !== undefined;

export const userHasSheepdogProjectAdmin = (
  userAuthMapping: Record<string, any> = {},
): boolean =>
...
export const projectCodeFromResourcePath = (resourcePath: string): string => {
  ...
  return split.length < 5 || split[1] !== 'programs' || split[3] !== 'projects'
    ? ''
    : split[4];
};

export const listifyMethodsFromMapping = (
  actions: { service: string; method: string }[],
): string[] => {
  const reducer = (
    accumulator: string[],
    currval: { service: string; method: string },
  ): string[] => accumulator.concat([currval.method]);
  return actions.reduce(reducer, []);
};

export const userHasDataUpload = (
...
export const userHasMethodForServiceOnResource = (
  ...
  return (
    actions !== undefined &&
    actions.some(
      (x: { service: string; method: string }) =>
...
export const userHasMethodOnAnyProject = (
  method: string,
  userAuthMapping: AuthzMapping = {},
): boolean => {
  const actionHasMethod = (x: { method: string }): boolean =>
    x.method === method;
  const actionArrays = Object.values(userAuthMapping);
  const hasMethod = actionArrays.some((x: { method: string }[]) =>
    x.some(actionHasMethod),
  );
  return hasMethod;
};

export const userHasCreateOrUpdateOnAnyProject = (
  userAuthMapping: AuthzMapping = {},
): boolean =>
  userHasMethodOnAnyProject('create', userAuthMapping) ||
