...
if you need to upgrade a run gen helm and see this error:
```bash
Error: UPGRADE FAILED: failed to create resource: admission webhook "validate.nginx.ingress.kubernetes.io" denied the request: host "localhost" and path "/" is already defined in ingress default/ngress-nginx-controller
```
Run the following:
```bash
