 ...
 */
function process(inputDir: string, outputDir: string): void {
  console.log('Loading JSON version of Gen3 dictionary');

  const schemaPath = path.join(inputDir, 'dictionary.json');
  ...
  const outputPath = path.join(outputDir, 'gdc_tooltips.json');
  fs.writeFileSync(outputPath, JSON.stringify(facetDescriptions, null, 2));

  console.log('Processing complete');
}

interface GetFacetsArgs extends Args {
  inpath: string;
  outpath: string;
}
 ...
 */
function main(): void {
  const { inpath, outpath }: GetFacetsArgs = getArgs({
    inpath: './',
    outpath: './',
  });
