...
const Footer: React.FC<FooterProps> = ({ bottomLinks, columnLinks, footerLogos, footerRightLogos }: FooterProps) => {
    ...
    <div className='bg-gen3-coal text-color-white p-4'>
      <div className='flex flex-row justify-end'>
        {
          (footerLogos || [[]]).map((col, index) => {
            return (
              <div key={`footer-col-${index}`} className={`flex flex-row gap-x-3 mr-8 ${col.length>1 ? 'justify-between' : ''}`}>
                {
                  col.map((logo) =>
      ...
      </div>
      {
        (footerRightLogos || [[]]).map((col, index) => {
          return (
            <div key={`footer-col-${index}`} className={`flex flex-row gap-x-6 mr-8 ${col.length>1 ? 'justify-end' : ''}`}>
              {
                col.map((logo) =>
                  <Image key={`${basePath}/icons/${logo.logo}`}
