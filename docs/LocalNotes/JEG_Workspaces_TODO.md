...
* Match wireframe
    * landing page ~~* Terminate Kernels~~
...
quay.io/cdis/gen3-vectis:gen3-vectisv6 --name kind-multi-node kind load docker-image quay.io/cdis/gen3-vectis:qa-goproxy
--name kind-multi-node kind load docker-image quay.io/cdis/multihead-workspace-proxy:feat_init-jeg-user-ownership-fix
...
Build docker file: stop and upen bash shell on error

```bash
 BUILDX_EXPERIMENTAL=1 docker buildx debug --invoke bash build -f Dockerfile.production .
```
